#!/usr/bin/env python3
"""Build a ChromaDB vector index of the Quran + Hadith for semantic search.

Each verse/hadith (citation + Arabic text) becomes a document. Embeddings come
from Ollama's bge-m3 (1024-dim, multilingual — handles Arabic). The index lives
in chroma/ and supports citation-grounded retrieval.

Usage:
  python ic_index.py            # build/refresh the index
  python ic_index.py --query "بِسْمِ اللَّهِ" --k 5
"""
...
def main():
    ...
    for i in range(start, len(entries), BATCH):
        ...
        ids = [f"e{i+j}" for j in range(len(batch))]
