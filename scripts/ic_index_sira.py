#!/usr/bin/env python3
"""Append the Sira (biography of Muhammad) layer to the islamic_corpus index.

Uses distinct IDs (r{idx}) and upsert, so re-running is safe.

Usage:
  python ic_index_sira.py            # append sira entries
  python ic_index_sira.py --query "..." --k 5
"""
import os, sys, json, time, urllib.request, urllib.error

BASE = os.path.dirname(os.path.abspath(__file__))
SIRA_TSV = os.path.join(BASE, 'text', 'sira.tsv')
...
def load_entries():
    entries = []
    if not os.path.exists(SIRA_TSV):
        return entries
    with open(SIRA_TSV, encoding='utf-8') as f:
        for line in f:
            line = line.rstrip('\n')
            if '\t' not in line:
                continue
...
def main():
    ...
    print(f"Loaded {len(entries)} sira entries")

    BATCH = 32
    for i in range(0, len(entries), BATCH):
        batch = entries[i:i+BATCH]
        ids = [f"r{i+j}" for j in range(len(batch))]
